...
def create_logger(log_dir, phase='train'):
    ...
    logging.getLogger('train').addHandler(console)

    file_handler = logging.handlers.RotatingFileHandler(final_log_file, maxBytes=(1024 ** 2 * 2), backupCount=3)
...
def mae(outputs: np.array, labels: np.array):
...
def eval_seg(pred,true_mask_p,threshold):
    ...
    if c == 2:
        ...
        for th in threshold:
            ...
            '''mae for numpy'''
            emae += mae(disc_pred,disc_mask)
            
        return eiou / len(threshold), edice / len(threshold), emae / len(threshold)
